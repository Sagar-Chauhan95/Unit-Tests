...
type CustomerRecord={customerId:number,
...
type Bank = {
    ...
    credit: (customerId:number, amound:number)=>void;
...
bank.saveTransaction=function(customerDetail:number, amount:number):void{
...
bank.debit= function(customerDetail:number, amount:number ):void{
...
bank.credit= function(customerDetail:number, amount:number):void{
    bank.transactionsDB[customerDetail-1].customerTransactions.push(amount);
}


bank.getBalance=function(customerDetail:number):number{
    let sum =0;
...
export function executor(str:string, num1:number,num2:number):number{
